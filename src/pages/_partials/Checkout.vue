          ...
          <modal name="checkout">
            ...
            <div class="px-md-5 my-4" v-else>
                <div class="col=12" v-if="me.name !== ''">
                    ...
                    <p><strong>Preço total:</strong> {{ totalCart | formatprice }}</p>
                    <p v-if="company.table.identify">
                      <strong>Mesa:</strong> {{ company.table.name }}
                    </p>
                    <div class="form-group">
                        <textarea name="comment" v-model="comment" class="form-control" cols="30" rows="2" placeholder="Comentário (Opcional)"></textarea>
                        <p></p>
                    </div>
  ...
  methods: {
    ...
    createOrder () {
      ...
      this.$store.dispatch(action, params)
                .then(order => {
                  this.$toast.success('Pedido realizado com Sucesso ! - Parabéns !!!')
                ...
                .catch(error => {
                  this.$toast.error('Falha ao realizar Pedido !!!')
                })
