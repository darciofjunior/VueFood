        ...
        <div class="row my-4">
          ...
          <div class="col-lg-4 col-md-6 mb-4" v-for="(product, index) in company.products.data" :key="index">
            <div :class="['card', 'h-100', {'disabled' : productInCart(product)}]">
              <a href="#">
                <img class="card-img-top" :src="product.image" alt="" width="100px" height="200px">
              </a>
              <div class="card-body">
                <h4 class="card-title">
                  <a href="#">{{ product.title }}</a>
                </h4>
                <h5>{{ product.price | formatprice }}</h5>
...
export default {
    ...
    created() {
        ...
        this.getCategoriesByCompany(this.company.uuid)
            .catch(response => this.$toast.error('Falha ao carregar as Categorias.'))
    ...
    methods: {
      ...
      loadProducts () {
        ...
        this.getProductsByCompany(params)
            .catch(response => this.$toast.error('Falha ao carregar os Produtos.'))
      },
